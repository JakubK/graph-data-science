...
For further instructions, see our https://neo4j.com/docs/graph-data-science/current/installation/[documentation].
Alternatively, you can use attached `docker-compose.yml` to build and start a container producing .jar file without having Java or any other dependency than Docker installed:
```
docker-compose build
docker-compose up
```
Once the job is done and container gets terminated, `build/distributions` directory should be present with artifact jar.
